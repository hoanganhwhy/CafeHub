...
import { 
  ArrowLeft, Plus, Edit, Trash2, Lock, Coffee, UtensilsCrossed, 
  IceCream, Shield, Sparkles, Save, X, AlertCircle, CheckCircle 
} from 'lucide-react';
  ...
  const handlePinSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (pin === correctPin) {
        ...
        showNotification('error', 'Mã PIN không chính xác!');
        setPin('');
      }
      setIsLoading(false);
    }, 1000);
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-red-50 via-pink-50 to-purple-50 flex items-center justify-center p-4">
        <div className="bg-white/80 backdrop-blur-lg rounded-3xl shadow-2xl p-10 max-w-md w-full mx-4 border border-gray-200/50">
          <div className="text-center mb-8">
            <div className="relative mb-6">
              <div className="bg-gradient-to-br from-red-500 to-pink-600 p-6 rounded-full w-20 h-20 mx-auto flex items-center justify-center shadow-2xl">
                <Lock className="w-10 h-10 text-white" />
              </div>
              <Shield className="w-6 h-6 text-red-400 absolute -top-1 -right-1 animate-pulse" />
            </div>
            <h1 className="text-3xl font-bold bg-gradient-to-r from-red-600 to-pink-600 bg-clip-text text-transparent mb-3">
              Quản lý Menu
            </h1>
            <p className="text-gray-600 text-lg">Nhập mã PIN để truy cập khu vực quản trị</p>
          </div>
          ...
          <form onSubmit={handlePinSubmit} className="space-y-6">
            <div className="relative">
              <input
                ...
                placeholder="••••"
                className="w-full p-4 border-2 border-gray-200 rounded-2xl text-center text-3xl font-mono tracking-[0.5em] focus:ring-4 focus:ring-red-500/20 focus:border-red-500 transition-all duration-300 bg-gradient-to-r from-white to-gray-50"
                maxLength={4}
                autoFocus
                disabled={isLoading}
              />
              <div className="absolute inset-0 bg-gradient-to-r from-red-500/10 to-pink-500/10 rounded-2xl opacity-0 group-focus-within:opacity-100 transition-opacity duration-300 pointer-events-none"></div>
            </div>
            ...
            <div className="flex gap-4">
              <button
                type="button"
                onClick={() => navigate('/')}
                className="flex-1 px-6 py-4 border-2 border-gray-300 text-gray-700 rounded-2xl hover:bg-gray-50 transition-all duration-300 font-medium"
                disabled={isLoading}
              >
                Quay lại
              </button>
              <button
                ...
                className="flex-1 px-6 py-4 bg-gradient-to-r from-red-500 to-pink-600 text-white rounded-2xl hover:from-red-600 hover:to-pink-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-red-500/30 font-medium"
              >
                {isLoading ? (
          ...
          <div className="mt-8 p-4 bg-gradient-to-r from-yellow-50 to-orange-50 border-2 border-yellow-200 rounded-2xl">
            <div className="flex items-center gap-2 mb-2">
              <AlertCircle className="w-5 h-5 text-yellow-600" />
              <span className="font-bold text-yellow-800">Gợi ý</span>
            </div>
            <p className="text-sm text-yellow-800">
              Mã PIN mặc định là <code className="bg-yellow-100 px-2 py-1 rounded font-mono font-bold">1234</code>
            </p>
          </div>
        </div>
      </div>

...
import { Search, Eye, Printer, ArrowLeft, Calendar, FileText, TrendingUp, DollarSign, ShoppingBag, Sparkles, Filter } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="flex items-center justify-between">
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600 font-medium">Theo dõi và quản lý tất cả đơn hàng</p>
            </div>
            
            <div className="flex items-center gap-2">
              <div className="bg-gradient-to-r from-green-100 to-emerald-100 px-4 py-2 rounded-xl">
                <span className="text-green-700 font-bold">{orders.length}</span>
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl border border-gray-200/50 p-8 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-3">Lọc theo ngày</label>
              <div className="relative group">
                      ...
                      <td className="px-8 py-6">
                        <div className="space-y-2">
                          {order.items.slice(0, 2).map((item, itemIndex) => (
                            <div key={item.id} className="flex items-center gap-2 bg-gradient-to-r from-gray-50 to-white px-3 py-2 rounded-lg">
                              <span className="text-2xl">{itemIndex === 0 ? '🍜' : '☕'}</span>
                          ...
                          {order.items.length > 2 && (
                            <div className="text-center py-2 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg">
                              <span className="text-blue-600 font-bold">+{order.items.length - 2} món khác</span>
                            </div>
                          )}
                        </div>
                      </td>
                      <td className="px-8 py-6 whitespace-nowrap">
                        <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-xl border border-green-200">
                          ...
                          <div className="text-xs text-gray-600 mt-1 space-y-1">
                            <div>💰 Nhận: <span className="font-medium">{formatCurrency(order.payment)}</span></div>
                            <div>💸 Thừa: <span className="font-medium">{formatCurrency(order.change)}</span></div>
                          </div>
                        </div>
                      </td>
                      <td className="px-8 py-6 whitespace-nowrap">
